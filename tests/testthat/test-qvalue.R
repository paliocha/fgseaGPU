...
test_that("fora exposes midP and applies discrete q-values", {
  ...
  expect_named(res, c(
    "pathway", "pval", "midP", "padj", "foldEnrichment",
...
test_that("small-m fgseaMultilevel padj falls back to BH (issue #2)", {
  # With ~40 pathways the Storey pi0 estimator is unreliable. Regression
  # test: pi0 must be in (0, 1], padj must be >= pval (q <= p is impossible
  # for a valid q-value), and padj must agree with BH within numerical
  # noise since small-m forces the BH fallback path.
  set.seed(1)
  stats <- setNames(rnorm(20000), paste0("g", 1:20000))
  pw <- lapply(1:40, function(i) sample(names(stats), sample(5:80, 1)))
  names(pw) <- paste0("set", 1:40)
  pw[[1]] <- names(sort(stats, decreasing = TRUE))[1:40]
  res <- fgseaMultilevel(pw, stats, minSize = 5, maxSize = 2000)
  ...
  bh <- stats::p.adjust(res$pval, method = "BH")
  expect_lt(max(abs(res$padj - bh)), 1e-9)
})

test_that("fgseaMultilevel NES is finite and sign-consistent (issue #3)", {
  set.seed(1)
  stats <- setNames(rnorm(20000), paste0("g", 1:20000))
  pw <- lapply(1:30, function(i) sample(names(stats), sample(10:80, 1)))
  names(pw) <- paste0("set", 1:30)
  pw[[1]] <- names(sort(stats, decreasing = TRUE))[1:40] # planted set
  res <- fgseaMultilevel(pw, stats, minSize = 5, maxSize = 2000)

  expect_true(all(is.finite(res$NES)))
  # Sign of NES tracks sign of ES (denominator is mean(|null|) > 0).
  nz <- res$ES != 0
  expect_true(all(sign(res$NES[nz]) == sign(res$ES[nz])))
  # Planted set lights up clearly.
  top <- res[res$pathway == "set1", ]
  expect_gt(abs(top$NES), 1.5)
})
...
test_that("Storey q-values agree with qvalue::qvalue when available", {
  ...
  pathways <- setNames(lapply(seq_along(p), function(i) {
    sample(names(stats), 20)
  }), paste0("pw", seq_along(p)))
