...
#include "../../src/fgsea_multilevel.h"
#include "../../src/fgsea_fora.h"
#include "../../src/fgsea_phenotype.h"
#include "../../src/fgsea_qvalue.h"
...
void test_hand_computed_es() {
    ...
    auto r = fgsea::calcEs(std::span<double const>(stats),
                           std::span<std::int32_t const>(pos),
                           1.0, fgsea::ScoreType::Std);
...
void test_zero_es_throws() {
    ...
    bool threw = false;
    try {
        fgsea::cpu::observedEs(std::span<double const>(stats),
                               std::span<std::int32_t const>(pos),
                               1.0, fgsea::ScoreType::Std);
...
void test_multilevel_basic() {
    ...
    auto res = fgsea::multilevel::runMultilevel(
        std::span<double const>(stats), pw, 1.0,
        fgsea::ScoreType::Pos, cfg);
    assert(res.size() == 1);
    assert(res[0].es > 0);
...
void test_midp_bounded_by_p() {
    ...
    auto mp = fgsea::qvalue::midP(p, pmf);
...
void test_phenotype_recovers_obvious_signal() {
    ...
    in.pathway_genes = {pw};
    in.nperm  = 200;
    in.metric = fgsea::phenotype::Metric::SignalToNoise;

    auto res = fgsea::phenotype::runPhenotype(in);
    assert(res.size() == 1);
    assert(res[0].es > 0);
    assert(res[0].pval > 0 && res[0].pval <= 1);
}
...
void test_phenotype_exact_via_gosper() {
    ...
    in.nperm = 50;
    in.seed  = 1;
    auto const r1 = fgsea::phenotype::runPhenotype(in);
    in.seed  = 9999;
    auto const r2 = fgsea::phenotype::runPhenotype(in);
